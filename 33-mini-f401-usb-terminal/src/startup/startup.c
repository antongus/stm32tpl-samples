...
*  @file startup.c
*
*  Startup file for STM32F401xx devices
*
...
static inline void initClocks()
{
	...
	PWR->CR |= (PWR->CR & ~PWR_CR_VOS) | PWR_CR_VOS_1;  // set VOS to Scale 2 mode (for high speed)

	RCC->CFGR |= 0
			| RCC_CFGR_HPRE_DIV1   // HCLK = SYSCLK. (Maximum frequency of the AHB domain is 84MHz.)
			| RCC_CFGR_PPRE2_DIV1  // PCLK2 = HCLK. (Maximum frequency of the high-speed APB2 domain is 84MHz.)
			| RCC_CFGR_PPRE1_DIV2  // PCLK1 = HCLK/2. (Maximum frequency of the low-speed APB1 domain is 42MHz.)
			;
